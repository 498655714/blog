    ...
    <form class="form-horizontal" id="modal-form" role="form" action="{{url('admin/article/'.$data[0]['art_id'])}}" method="post" >
            ...
            <div class="col-sm-9">
                <select name="cate_id" width="150px">
                    ...
                    @foreach($cates as $key=>$val)
                        <option value="{{$val['cate_id']}}" @if(isset($data[0]['cate_id']) && ($val['cate_id'] == $data[0]['cate_id'])) selected @endif>{{$val['_cate_name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-6">
                @foreach($tags as $tag=>$value)
                    <label>
                        <input name="art_tag[]" class="ace" type="checkbox"
                               @if(isset($data[0]['art_tag']) && !empty($data[0]['art_tag']))
                                   @foreach($data[0]['art_tag'] as $tag)
                                        @if($tag == $value['tag_id'])
                                            checked
                                        @endif
                                   @endforeach
        ...
        <div class="clearfix form-actions">
            <div class="col-md-offset-3 col-md-9">
                ...
                &nbsp; &nbsp; &nbsp;
                <button class="btn" type="reset" onclick="javascrtpt:window.location.href='{{url('admin/article')}}'">
